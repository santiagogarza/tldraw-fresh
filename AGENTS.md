...
Dependency install is handled automatically by the startup update script (`yarn install`, which also runs the `postinstall` husky + `refresh-assets` steps). The notes below are durable, non-obvious caveats for running things in this environment; standard commands live in the sections above and in root `package.json`.

- Node version: the environment's default `node` is Node 22 (resolved from `/exec-daemon/node`, ahead of nvm on `PATH`). Root `engines` says `^20.0.0`, but the repo targets Node 22 (`@types/node` is `^22`) and install, `yarn lint`, `yarn test`, `yarn typecheck`, and `yarn dev` all work on Node 22. `nvm` also has Node 20 available if a task ever needs it, but overriding `PATH` is usually unnecessary.
- Running the SDK: `yarn dev` serves the examples app at `http://localhost:5420` and also starts local Cloudflare workers via Wrangler/Miniflare — `apps/bemo-worker` on `8989` (multiplayer) and `apps/dotcom/image-resize-worker` on `8786`. Run it as a long-lived process (e.g. tmux), not a one-shot command.
- Wrangler noise is expected: on startup the workers log `Unable to fetch the \`Request.cf\` object`and`read ECONNRESET`because outbound Cloudflare network access is restricted. These are non-fatal — each worker still prints`Ready on http://localhost:...` and works locally.
- Test filtering: this Vitest version does not accept `--grep`. Filter by file (`yarn test run <path-or-substring>`) or by test name (`yarn test run -t "pattern"`) instead.
- `yarn typecheck` uses `tsgo` (the native TypeScript preview) and is fast.
- `yarn lint` may report the generated bundles `packages/tldraw/tldraw.css` and `packages/namespaced-tldraw/tldraw.css` as having format issues. These are generated by `refresh-assets` and are pre-existing; do not hand-format them.
- The full tldraw.com stack (`yarn dev-app`) additionally needs Docker/Postgres and Clerk keys; it is not required for core SDK development via `apps/examples`.
