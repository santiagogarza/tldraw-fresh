...
setup-demo-bug: ready for demo.
  - local app (with planted bug):  ${URL}
...
Next steps:
  1. Open ${URL}, draw a shape, select it, click a color swatch. Nothing changes. That's the bug.
  2. In Slack #santi-bug-automation, post a bug report mentioning @Cursor.
  3. The agent will thread its repro/screenshots/fix/PR under your message.
